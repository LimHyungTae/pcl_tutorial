...
export const ko: LocaleDict = {
  ...
  lec11: {
    ...
    pause: "⏸ Pause",
    converged: "✓ 수렴",
    statusIterating: "Iterating · fitness {fitness} · {pairs} pairs",
    statusOk: "수렴 성공 · fitness {fitness} · {pairs} pairs",
    statusWarn: "수렴은 했지만 fitness {fitness}가 큼 — local minimum에 빠진 것으로 보임 (초기 pose offset을 줄여보세요).",
    statusFail: "중단됨 · max correspondence distance 안에서 {pairs}개 pair만 발견됨.",
  ...
  chapters: {
    ...
    "genz-icp": {
      ...
      about:
        "GenZ-ICP (Lee et al., RA-L, 2024)은 모든 correspondence를 target 주변의 local planarity 기준으로 planar (point-to-plane) 또는 non-planar (point-to-point)로 분류하고, 한 번의 6-DoF Gauss-Newton step에서 두 residual을 적응적 가중치 α = #planar / (#planar + #non_planar)로 결합하여 frame을 local map에 정합합니다. Planar pair는 blue (RGB 0, 119, 187), non-planar pair는 magenta (RGB 238, 51, 119)로 그려집니다.",
      params: [
